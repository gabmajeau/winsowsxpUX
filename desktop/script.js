...
function openObjetivo() {
  const win = document.getElementById("janelaObjetivo");
  if (!win) return console.error("janelaObjetivo não encontrada");
  ...
  if (!window.__objetivoInitialized) initObjetivo();
  window.__objetivoInitialized = true;
}

function initObjetivo() {
  const win = document.getElementById("janelaObjetivo");
  const header = document.getElementById("janelaObjetivoHeader");
  const closeImg = document.getElementById("closeObjetivoBtn");

  if (!win || !header)
    return console.error("elementos de janela não encontrados");

  if (closeImg) {
    closeImg.addEventListener("mouseover", () => {
      if (closeImg.dataset.hover) closeImg.src = closeImg.dataset.hover;
    });
    closeImg.addEventListener("mouseleave", () => {
      if (closeImg.dataset.normal) closeImg.src = closeImg.dataset.normal;
    });
    closeImg.addEventListener("mousedown", () => {
      if (closeImg.dataset.click) closeImg.src = closeImg.dataset.click;
    });
    closeImg.addEventListener("mouseup", (e) => {
      if (closeImg.dataset.normal) closeImg.src = closeImg.dataset.normal;
